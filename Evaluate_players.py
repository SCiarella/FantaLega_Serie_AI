...
if __name__ == "__main__":
    giocatori_urls = []
    if not os.path.exists("./Players_evaluation/giocatori_urls.txt"):
        for i in tqdm(range(0, len(ruoli), 1)):
            lista = get_giocatori(ruoli[i])
            [giocatori_urls.append(el) for el in lista]
        with open(r"./Players_evaluation/giocatori_urls.txt", "w") as fp:
            for item in giocatori_urls:
                fp.write("%s\n" % item)
            logger.debug("URL scritti")
    else:
        logger.debug("Leggo la lista giocatori")
        with open("./Players_evaluation/giocatori_urls.txt", "r") as fp:
            giocatori_urls = fp.readlines()

    if not os.path.exists("./Players_evaluation/giocatori.csv"):
        giocatori = []
        ...
        df.to_csv("./Players_evaluation/giocatori.csv", index=False)
        
        logger.debug("CSV scritto")
    else:
        logger.debug("Leggo il dataset giocatori")
        df = pd.read_csv("./Players_evaluation/giocatori.csv")
    ...
    df.to_excel("Players_evaluation/giocatori_excel.xls")
    logger.debug("Finito!")
